...
// ─── Konstanta Kalibrasi
// ────────────────────────────────────────────────────── RL (kΩ) dan rasio
// Rs/R0 di udara bersih per sensor Nilai default dari datasheet — sesuaikan
// jika sirkuit berbeda
static const float SENSOR_RL[] = {
    20.0,  // TGS822
    20.0,  // MQ135
    200.0, // MQ3  (RL = 200 kΩ, sesuai datasheet MQ-3)
    20.0,  // TGS2611
    20.0,  // TGS2620
    20.0,  // TGS2600
...
static const float RATIO_AIR[] = {
    17.0, // TGS822
    3.6,  // MQ135
    60.0, // MQ3  (Rs/R0 di udara bersih, sesuai datasheet MQ-3)
    19.0, // TGS2611
    21.0, // TGS2620
    10.0, // TGS2600
...
SensorArray::SensorArray()
    : ads1_(I2C_ADDR_ADS1), ads2_(I2C_ADDR_ADS2), ads3_(I2C_ADDR_ADS3),
      ads4_(I2C_ADDR_ADS4),
...
bool SensorArray::begin() {
  ...
  if (hasAds1_) {
    ...
    Serial.println(F("{\"warn\":\"ADS1115 #1 (0x48) tidak terdeteksi\"}"));
  }
  ...
  if (hasAds2_) {
    ...
    Serial.println(F("{\"warn\":\"ADS1115 #2 (0x49) tidak terdeteksi\"}"));
  }

  if (hasAds3_) {
    ads3_.setGain(ADS_GAIN);
    foundCount++;
  } else {
    Serial.println(F("{\"warn\":\"ADS1115 #3 (0x4A) tidak terdeteksi\"}"));
  }

  if (hasAds4_) {
    ads4_.setGain(ADS_GAIN);
    foundCount++;
  } else {
    Serial.println(F("{\"warn\":\"ADS1115 #4 (0x4B) tidak terdeteksi\"}"));
  }
  ...
  mq3_.setRL(SENSOR_RL[SENSOR_MQ3]);
  mq8_.setRegressionMethod(1);
  mq8_.setRL(SENSOR_RL[SENSOR_MQ8]);

  // ── Konfigurasi TGS sensors ──────────────────────────────────────────────
  tgs822_.setRL(SENSOR_RL[SENSOR_TGS822]);
  tgs822_.setRatioAir(RATIO_AIR[SENSOR_TGS822]);
  tgs2620_.setRL(SENSOR_RL[SENSOR_TGS2620]);
  tgs2620_.setRatioAir(RATIO_AIR[SENSOR_TGS2620]);
  tgs2611_.setRL(SENSOR_RL[SENSOR_TGS2611]);
  tgs2611_.setRatioAir(RATIO_AIR[SENSOR_TGS2611]);
  tgs2600_.setRL(SENSOR_RL[SENSOR_TGS2600]);
  tgs2600_.setRatioAir(RATIO_AIR[SENSOR_TGS2600]);
  tgs2602_.setRL(SENSOR_RL[SENSOR_TGS2602]);
  tgs2602_.setRatioAir(RATIO_AIR[SENSOR_TGS2602]);
  tgs813_.setRL(SENSOR_RL[SENSOR_TGS813]);
  tgs813_.setRatioAir(RATIO_AIR[SENSOR_TGS813]);
  tgs816_.setRL(SENSOR_RL[SENSOR_TGS816]);
  tgs816_.setRatioAir(RATIO_AIR[SENSOR_TGS816]);
...
bool SensorArray::loadCalibration() {
  if (EEPROM.read(EEPROM_MAGIC_ADDR) != EEPROM_MAGIC_VALUE) {
    Serial.println(
        F("{\"error\":\"Sensor belum dikalibrasi. "
...
void SensorArray::readAll() {
  ...
  if (hasAds2_) {
    ...
    adc_[SENSOR_TGS2611] = 0;
    adc_[SENSOR_TGS2620] = 0;
  }
  ...
  if (hasAds3_) {
    ...
    adc_[SENSOR_TGS2600] = 0;
    adc_[SENSOR_TGS2602] = 0;
  }
...
// ─────────────────────────────────────────────────────────────────────────────
void SensorArray::printJsonData(const char *phase, uint32_t cycle,
                                uint32_t sampleIdx) {
  Serial.print(F("{\"phase\":\""));
  Serial.print(phase);
  Serial.print(F("\",\"cycle\":"));
  Serial.print(cycle);
  Serial.print(F(",\"sample_idx\":"));
  Serial.print(sampleIdx);
  Serial.print(F(",\"timestamp\":"));
  Serial.print(millis());

  // 10 ADC values
  Serial.print(F(",\"adc_tgs822\":"));
  Serial.print(adc_[SENSOR_TGS822]);
  Serial.print(F(",\"adc_mq135\":"));
  Serial.print(adc_[SENSOR_MQ135]);
  Serial.print(F(",\"adc_mq3\":"));
  Serial.print(adc_[SENSOR_MQ3]);
  Serial.print(F(",\"adc_tgs2611\":"));
  Serial.print(adc_[SENSOR_TGS2611]);
  Serial.print(F(",\"adc_tgs2620\":"));
  Serial.print(adc_[SENSOR_TGS2620]);
  Serial.print(F(",\"adc_tgs2600\":"));
  Serial.print(adc_[SENSOR_TGS2600]);
  Serial.print(F(",\"adc_tgs2602\":"));
  Serial.print(adc_[SENSOR_TGS2602]);
  Serial.print(F(",\"adc_mq8\":"));
  Serial.print(adc_[SENSOR_MQ8]);
  Serial.print(F(",\"adc_tgs813\":"));
